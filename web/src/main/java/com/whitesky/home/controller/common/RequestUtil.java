...
public class RequestUtil {
	...
	public static String getIpAddr(HttpServletRequest request) {
		...
		if (StringUtil.isBlank(ipString) || "unknown".equalsIgnoreCase(ipString)) {
			...
			ipString = ipString.equals("0:0:0:0:0:0:0:1") ? "localhost" : ipString;
		}
	 ...
	 * @return
	 */
	private static String callCmd(String[] cmd, String encoding) {
		String result = "";
		String line = "";
		try {
			Process proc = Runtime.getRuntime().exec(cmd);
			InputStreamReader is = new InputStreamReader(proc.getInputStream(), encoding==null?"utf-8":encoding);
	...
	private static String callCmd(String[] cmd, String[] another, String encoding) {
		String result = "";
		String line = "";
		try {
	...
	private static String filterMacAddress(final String ip, final String sourceString, final String macSeparator) {
		...
		while (matcher.find()) {
			...
			if (sourceString.indexOf(ip) <= sourceString.lastIndexOf(result)) {
	...
	private static String getMacInWindows(final String ip) {
		...
		String cmdResult = callCmd(cmd, another, "GBK");
	...
	private static String getMacInLinux(final String ip) {
		...
		String cmdResult = callCmd(cmd, "utf-8");
		result = filterMacAddress(ip, cmdResult, ":");
		return result;
	}
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		System.out.println(RequestUtil.getMacAddress("192.168.1.185"));
	}
